...
export const taskSchema=z.object({title:z.string().trim().min(2).max(160),projectId:z.string().min(1),status:z.enum(["Backlog","In progress","Review","Done"]).default("Backlog"),priority:z.enum(["Low","Medium","High"]).default("Medium"),assignee:z.string().trim().min(1).max(80),due:z.string().date()});
