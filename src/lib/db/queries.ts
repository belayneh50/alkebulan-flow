...
export async function getWorkspaceData(workspaceId:string){const db=await getDatabase();
 ...
 db.prepare("SELECT id,actor,action,target,kind,created_at createdAt FROM activities WHERE workspace_id=? ORDER BY created_at DESC LIMIT 20").all<Omit<Activity,"time">&{createdAt:string}>(workspaceId),
