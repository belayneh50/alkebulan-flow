...
import { createDemoBrief, projectBriefSchema } from "@/lib/ai/project-brief";
...
export async function POST(request: NextRequest) {
  ...
  if (process.env.GOOGLE_GENERATIVE_AI_API_KEY) {
    try {
      ...
      if (!project) return Response.json({ ...createDemoBrief(parsed.data.projectId), provider: "fallback" });
      const client = data.clients.find(item => item.id === project.clientId);
      const tasks = data.tasks.filter(item => item.projectId === project.id).map(({ title, status, priority, due }) => ({ title, status, priority, due }));
      const context = JSON.stringify({ project: { name: project.name, status: project.status, progress: project.progress, due: project.due, budget: project.budget }, client: client?.company, tasks });
      const result = await generateText({ model: google(process.env.GEMINI_MODEL || "gemini-3.6-flash"), output: Output.object({ schema: projectBriefSchema }), system: "You are Flow AI. Write a concise, strictly factual operations brief from the supplied project data only. Never invent facts, names, dates, or contact details.", prompt: `Project data: ${context}\n\nReturn a factual summary of current state, two realistic risks grounded in the tasks and deadline, three concrete next actions, and a professional client update addressed to ${client?.company ?? "the client"}.` });
      return Response.json({ ...result.output, provider: "gemini" });
    } catch (error) {
      console.error("[ai/brief] Gemini request failed, serving deterministic fallback:", error);
    }
  }
